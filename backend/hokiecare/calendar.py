...
def validate_stored_slot(slot):
    if not slot['active'] or slot['version'] != s.CONFIG['version'] or datetime.fromisoformat(slot['starts']) <= datetime.now(timezone.utc):
...
def rules(service_id: str):
    return {'service': s.service(service_id), 'timezone': s.CONFIG['timezone'],
            ...
            'exceptions': s.CONFIG['exceptions']}
    ...
    if mode == 'demo':
        with b.database() as db:
            ...
            stored = [dict(r) for r in db.execute('''SELECT s.* FROM slots s WHERE service_id=? AND local_date>=? AND local_date<?
                AND (active=1 OR EXISTS(SELECT 1 FROM appointments a WHERE a.slot_id=s.id AND a.status='reserved'))
                ORDER BY starts''',(service_id,str(start),str(end))).fetchall()]
...
class ProposalRequest(b.ReserveRequest):
    ...
    booking_name: str = Field(default='',max_length=80)
...
def prepare_review(body: ProposalRequest,request: Request,intake_key=None,waitlist_id=None):
    ...
    with b.database() as db:
        ...
        if old:
            if intake_key and old['intake_key']!=intake_key:
                raise HTTPException(409,'Request key already belongs to another intake.')
...
def confirm(ident:str,request:Request,response:Response):
    b.require_write(request)
    with b.database() as db:
        db.execute('BEGIN IMMEDIATE'); b.cleanup(db)
        owner=b.session_id(request,db)
        ...
        if p['result_id']:
            ...
            return b.get_appointment(db,owner,p['result_id'])
        ...
        db.execute('UPDATE appointments SET retain_until=? WHERE id=?',(expiry,result))
        if p['booking_name']:
...
@router.delete('/proposals/{ident}')
def discard(ident:str,request:Request):
    b.require_write(request)
    with b.database() as db:
        db.execute('BEGIN IMMEDIATE')
        owner=b.session_id(request,db)
        db.execute('UPDATE proposals SET expires=0 WHERE id=? AND owner=? AND result_id IS NULL',(ident,owner))
