...
**Grounded assistant:** `POST /api/assistant/booking` accepts `message` and an optional bounded `history` of user/assistant turns (max 10, 2400 characters each). It retrieves the Databricks gold directory before inference; an outage returns a visible error, not local fallback facts. Responses include generated prose, verified source cards, fresh demo inventory and validated internal navigation. `DELETE /api/assistant/booking` clears only conversation preferences, not appointments. Raw turns stay in page memory and are sent to Databricks for context. No search/browser tools or free-form SQL are exposed. Two model calls are typical; one bounded grounding repair may add a third. Existing six-per-session/twenty-global requests per minute and two concurrent assistant runs remain. Live authored evaluation: `python scripts/evaluate_assistant.py URL --output runtime/assistant-eval.json`. Review semantic quality, not just HTTP status.
...
Server variables: `DATABRICKS_HOST`, `DATABRICKS_AUTH_TYPE=oauth-m2m`, `DATABRICKS_CLIENT_ID`, `DATABRICKS_CLIENT_SECRET`, `DATABRICKS_WAREHOUSE_ID`, `PORT=8000`. Never set `DATABRICKS_CONFIG_PROFILE` in production. The assistant defaults to the verified `databricks-qwen3-next-80b-a3b-instruct` endpoint; override with `DATABRICKS_CHAT_ENDPOINT` only after validating tool calls. The demo database uses a Railway volume mounted at `/data`; keep one replica. For local HTTP set `HOKIECARE_COOKIE_SECURE=false`; production defaults to Secure cookies. See [booking operations and validation](BOOKING_IMPLEMENTATION.md).
