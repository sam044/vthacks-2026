...
export function CareIntake({visible}:{visible:boolean}) {
  ...
  return <section className="care-intake" aria-label="Appointment request">
    ...
    {hasResult ? <div className="intake-result" aria-live="polite">
      <h2 ref={heading} tabIndex={-1}>{b.saved?'Appointment saved':result?.outcome==='urgent_support'?'Find immediate support':!b.review&&!result?.slots?.length&&result?.waitlist_options?.length?'this time is taken, would you like to join the waitlist?':result?.reason==='appointment_conflict'?'You already have an appointment at this time.':result?.outcome==='no_match'?'Let’s adjust your request':b.review?'Review your appointment':'Choose your appointment time'}</h2>
